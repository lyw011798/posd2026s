...
TEST(LibraryItemTest, AddItemThrowsException) {
    // Arrange
    std::unique_ptr<LibraryItem> item = std::make_unique<MockLibraryItem>(1, "Test", "Author");
    std::unique_ptr<LibraryItem> child = std::make_unique<MockLibraryItem>(2, "Child", "Child Author");

    // Act & Assert
    EXPECT_THROW(item->add_item(child.get()), std::runtime_error);
}
